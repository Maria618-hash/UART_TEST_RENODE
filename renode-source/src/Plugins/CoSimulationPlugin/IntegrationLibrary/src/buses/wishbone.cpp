...
void Wishbone::write(int width, uint64_t addr, uint64_t value)
{
    ...
    *wb_stb = 1;
     ...
     // For Uart implementaion 
    *wb_addr = (uint8_t)((addr / granularity) & ((1 << addr_lines) - 1));   
    
    // For SPI implementation
      if (width == 4) {  // 32-bit operation  
        *(uint32_t*)wb_wr_dat = (uint32_t)value;  
    } else {  
        *wb_wr_dat = value;  
    }  
    //
